...
// newAuction builds a new auction from a map of seat bids, applying the 4-level tie-break rule
// when selecting the overall winner per impression:
//  1. Highest price
//  2. Higher seat weight (from seatWeights; absent = 0)
//  3. Lower observed latency (from seatLatency in ms; 0 = unknown, treated as ∞)
//  4. Lexicographically smaller stable key (bidder + ":" + bid.ID)
func newAuction(seatBids map[openrtb_ext.BidderName]*entities.PbsOrtbSeatBid, numImps int, preferDeals bool, seatWeights map[openrtb_ext.BidderName]int, seatLatency map[openrtb_ext.BidderName]int) *auction {
	...
	for bidderName, seatBid := range seatBids {
		if seatBid != nil {
			for _, bid := range seatBid.Bids {
				...
				if !ok || bidBeats(bid.Bid, bidderName, wbid.Bid, winningBidders[bid.Bid.ImpID], preferDeals, seatWeights, seatLatency) {
	...
	return &auction{
		...
		seatLatency:     seatLatency,
	}
}

// bidBeats returns true if the incoming bid (newBid from newBidder) should displace
// the current leading bid (curBid from curBidder) using the 4-level tie-break rule:
//  1. Highest price wins
//  2. Higher seat weight wins (absent = 0)
//  3. Lower latency wins (0 = unknown, treated as ∞)
//  4. Lexicographically smaller stable key (bidder + ":" + bid.ID) wins
func bidBeats(
	newBid *openrtb2.Bid, newBidder openrtb_ext.BidderName,
	curBid *openrtb2.Bid, curBidder openrtb_ext.BidderName,
	preferDeals bool,
	seatWeights map[openrtb_ext.BidderName]int,
	seatLatency map[openrtb_ext.BidderName]int,
) bool {
	if preferDeals {
		if len(curBid.DealID) > 0 && len(newBid.DealID) == 0 {
			return false
		}
		if len(curBid.DealID) == 0 && len(newBid.DealID) > 0 {
			return true
	...
	// Tier 1: price
	if newBid.Price != curBid.Price {
		return newBid.Price > curBid.Price
	}

	// Tier 2: seat weight (higher wins)
	newW, curW := seatWeights[newBidder], seatWeights[curBidder]
	if newW != curW {
		return newW > curW
	}

	// Tier 3: latency (lower wins; 0 = unknown → treated as ∞)
	newL, curL := seatLatency[newBidder], seatLatency[curBidder]
	if newL != curL {
		if newL == 0 {
			return false
		}
	...
	// Tier 4: stable key (lexicographic on bidder + ":" + bid.ID)
	return string(newBidder)+":"+newBid.ID < string(curBidder)+":"+curBid.ID
}

// isNewWinningBid is retained for same-bidder multibid sorting where tie-break
// beyond price is not needed.
...
type auction struct {
	...
	// seatWeights maps bidder names to a priority weight used in tie-breaking (higher = preferred).
	seatWeights map[openrtb_ext.BidderName]int
	// seatLatency maps bidder names to their observed response latency in ms (lower = preferred; 0 = unknown).
	seatLatency map[openrtb_ext.BidderName]int
}
