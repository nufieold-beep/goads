...
func (e *exchange) HoldAuction(ctx context.Context, r *AuctionRequest, debugLog *DebugLog) (*AuctionResponse, error) {
	...
	if anyBidsReturned {
		...
		if targData != nil {
			...
			// Build per-seat latency map from adapter response times.
			seatLatency := make(map[openrtb_ext.BidderName]int, len(adapterExtra))
			for bidder, extra := range adapterExtra {
				if extra != nil {
					seatLatency[bidder] = extra.ResponseTimeMillis
				}
			}

			// Build per-seat weight map from account configuration.
			seatWeights := make(map[openrtb_ext.BidderName]int, len(r.Account.SeatWeights))
			for seat, w := range r.Account.SeatWeights {
				seatWeights[openrtb_ext.BidderName(seat)] = w
			}

			// A non-nil auction is only needed if targeting is active. (It is used below this block to extract cache keys)
			auc = newAuction(adapterBids, len(r.BidRequestWrapper.Imp), targData.preferDeals, seatWeights, seatLatency)
	...
	// Add per-bidder warnings for each bidder blocked by privacy settings.
	for _, err := range errs {
		if privacyErr, ok := err.(*errortypes.PrivacyBlockedWarning); ok {
			bidderName := openrtb_ext.BidderName(privacyErr.BidderName)
			bidResponseExt.Warnings[bidderName] = append(
				bidResponseExt.Warnings[bidderName],
				openrtb_ext.ExtBidderMessage{
					Code:    errortypes.BidderBlockedByPrivacySettings,
					Message: "blocked by privacy settings",
				},
			)
		}
	}
