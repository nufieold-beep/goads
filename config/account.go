...
type Account struct {
	...
	// SeatWeights maps seat (bidder) names to an integer priority weight used during
	// tie-breaking when two bids share the same price. Higher values are preferred.
	SeatWeights map[string]int `mapstructure:"seat_weights" json:"seat_weights,omitempty"`
}
