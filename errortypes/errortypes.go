...
// PrivacyBlockedWarning is raised when a bidder's request is blocked by a privacy enforcement.
// It carries the bidder name so callers can surface per-bidder warnings in the auction response.
type PrivacyBlockedWarning struct {
	Message     string
	WarningCode int
	BidderName  string
}

func (err *PrivacyBlockedWarning) Error() string {
	return err.Message
}

func (err *PrivacyBlockedWarning) Code() int {
	return err.WarningCode
}

func (err *PrivacyBlockedWarning) Severity() Severity {
	return SeverityWarning
}
