...
const (
	...
	AdomainParameter         = "adomain"
	ImpIDParameter           = "impid"
	WidthParameter           = "w"
	HeightParameter          = "h"
	PbsVersionParameter      = "pbs_version"
)
...
func ParseEventRequest(r *http.Request) (*analytics.EventRequest, []error) {
	...
	// price (optional)
	if p := r.URL.Query().Get(PriceParameter); p != "" {
	...
	// adomain (optional)
	event.Adomain = r.URL.Query().Get(AdomainParameter)

	// impid (optional)
	event.ImpID = r.URL.Query().Get(ImpIDParameter)

	// width (optional)
	if w := r.URL.Query().Get(WidthParameter); w != "" {
		if width, err := strconv.ParseInt(w, 10, 64); err == nil {
			event.Width = width
		}
	}

	// height (optional)
	if h := r.URL.Query().Get(HeightParameter); h != "" {
		if height, err := strconv.ParseInt(h, 10, 64); err == nil {
			event.Height = height
		}
	}

	// pbs_version (optional)
	event.PbsVersion = r.URL.Query().Get(PbsVersionParameter)
...
func optionalParameters(request *analytics.EventRequest) string {
	...
	if request.Price > 0 {
		r.Add(PriceParameter, strconv.FormatFloat(request.Price, 'f', -1, 64))
	}

	if request.Adomain != "" {
		r.Add(AdomainParameter, request.Adomain)
	}

	if request.ImpID != "" {
		r.Add(ImpIDParameter, request.ImpID)
	}

	if request.Width > 0 {
		r.Add(WidthParameter, strconv.FormatInt(request.Width, 10))
	}

	if request.Height > 0 {
		r.Add(HeightParameter, strconv.FormatInt(request.Height, 10))
	}

	if request.PbsVersion != "" {
		r.Add(PbsVersionParameter, request.PbsVersion)
	}
